...
::: tip 检测机制
冲突检测通过读取 Plugin 资源 `plugin-comment-next` 的 `status.phase` 判断是否已启用（STARTED），并通过 SchemeManager 检测 `commentnext.halo.run` 组下扩展是否注册作为兜底，不直接引用 Comment Next 插件的 API 类，避免未安装时触发 `NoClassDefFoundError`。
:::
